...
pcp_sheet = get_or_create_worksheet(spreadsheet, "Pilot Coating Process Tbl", [
    "PCoating_ID", "Solution ID", "Date", "Box Temperature", "Box RH", "N2 flow",
...
dcp_sheet = get_or_create_worksheet(spreadsheet, "Dip Coating Process Tbl", [
    "DCoating_ID", "Solution ID", "Batch Fiber ID", "UnCoatedSpool ID", "Date", "Box Temperature", "Box RH", "N2 flow",
    "Number of fibers", "Coating Speed", "Annealing time", "Annealing Temp", "Operator Initials", "Ambient Temp", "Ambient %RH", "Notes"
])

ct_sheet = get_or_create_worksheet(spreadsheet, "Coater Tension Tbl", [
    "Tension_ID", "PCoating_ID", "Payout Location", "Tension (g)", "Notes"
])

csm_sheet = get_or_create_worksheet(spreadsheet, "Coating Solution Mass Tbl", [
    "SolutionMass_ID", "Solution ID", "Date & Time", "DCoating_ID", "PCoating_ID",
    "Solution Mass", "Operators Initials", "Notes"
])

# ----------- STREAMLIT UI -----------
st.title(":test_tube: Coating Process Data Entry")
...
with tabs[0]:
    ...
    with st.form("pilot_form"):
        ...
        pilot = {
            ...
            "layer_type": st.selectbox("Coating Layer Type", ["GL", "AL", "PL"], key="p_layer"),
...
with tabs[1]:
    ...
    with st.form("dip_form"):
        ...
        dip = {
            "solution_id": st.selectbox("Solution ID", solution_ids, key="d_solution"),
            ...
            "box_temp": st.number_input("Box Temperature", min_value=0.0, key="d_box_temp"),
            "box_rh": st.number_input("Box RH", min_value=0.0, key="d_box_rh"),
            "n2_flow": st.number_input("N2 flow", min_value=0.0, key="d_n2"),
            "num_fibers": st.number_input("Number of fibers", min_value=0, key="d_fibers"),
            "coating_speed": st.number_input("Coating Speed", min_value=0.0, value=1.0, key="d_speed"),  # default value
            "anneal_time": st.number_input("Annealing time", min_value=0.0, value=10.0, key="d_anneal_time"), # default
            "anneal_temp": st.number_input("Annealing Temp", min_value=0.0, value=60.0, key="d_anneal_temp"), # default
            "operator": st.text_input("Operator Initials", key="d_op"),
            "ambient_temp": st.number_input("Ambient Temp", min_value=0.0, key="d_amb_temp"),
            "ambient_rh": st.number_input("Ambient %RH", min_value=0.0, key="d_amb_rh"),
            "notes": st.text_area("Notes", key="d_notes")
        }
        if st.form_submit_button("Submit Dip Coating"):
            dcp_sheet.append_row([
                dcoating_id, dip["solution_id"], dip["batch_fiber"], dip["spool"], dip["date"].strftime("%Y-%m-%d"),
                dip["box_temp"], dip["box_rh"], dip["n2_flow"], dip["num_fibers"], dip["coating_speed"],
                dip["anneal_time"], dip["anneal_temp"], dip["operator"], dip["ambient_temp"], dip["ambient_rh"], dip["notes"]
            ])
...
with tabs[2]:
    ...
    if st.button("Submit All Tension Data"):
        for pcoating_id, payout, tension_val, note in st.session_state.tension_list:
            tid = get_next_id(ct_sheet, "Tension_ID")
            ct_sheet.append_row([tid, pcoating_id, payout, tension_val, note])
...
with tabs[3]:
    ...
    if st.session_state.mass_list:
        ...
        st.table(pd.DataFrame(st.session_state.mass_list, columns=["Solution ID", "Date & Time", "DCoating ID", "PCoating ID", "Solution Mass", "Initials", "Notes"]))
...
safe_preview("Coater Tension", ct_sheet.get_all_records(), "PCoating_ID")  # No date column, but shows by PCoating

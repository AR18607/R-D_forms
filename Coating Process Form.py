...
# --- Load worksheets with exact headers ---
...
payout_locations = ["A", "B", "C", "D", "Other"]  # Static example, replace if you have dynamic list
...
with tab1:
    ...
    with st.form("pilot_coating_form", clear_on_submit=True):
        ...
        pcoat_date = st.date_input("Date", datetime.today(), key="pilot_date")
        box_temp = st.number_input("Box Temperature (°C)", min_value=0.0, key="pilot_box_temp")
        box_rh = st.number_input("Box RH (%)", min_value=0.0, key="pilot_box_rh")
        n2 = st.number_input("N2 flow", min_value=0.0, key="pilot_n2")
        load_cell = st.number_input("Load cell slope", min_value=0.0, key="pilot_load_cell")
        num_fibers = st.number_input("Number of fibers", min_value=0, key="pilot_num_fibers")
        coat_speed = st.number_input("Coating Speed", min_value=0.0, key="pilot_coat_speed")
        tower1 = st.number_input("Tower 1 set point", min_value=0.0, key="pilot_tower1")
        tower1_ent = st.number_input("Tower 1 entry temperature", min_value=0.0, key="pilot_tower1_ent")
        tower2 = st.number_input("Tower 2 set point", min_value=0.0, key="pilot_tower2")
        tower2_ent = st.number_input("Tower 2 entry temperature", min_value=0.0, key="pilot_tower2_ent")
        layer_type = st.selectbox("Coating Layer Type", ["GL", "AL", "PL"], key="pilot_layer_type")
        op_init = st.text_input("Operator Initials", key="pilot_op_init")
        amb_temp = st.number_input("Ambient Temp", min_value=0.0, key="pilot_amb_temp")
        amb_rh = st.number_input("Ambient %RH", min_value=0.0, key="pilot_amb_rh")
        notes = st.text_area("Notes", key="pilot_notes")

        pilot_submit = st.form_submit_button("Submit Pilot Coating Entry")

    if pilot_submit:
        pcoat_ws.append_row([
            next_pcoat_id, pcoat_sol, pcoat_date.strftime("%Y-%m-%d"), box_temp, box_rh, n2, load_cell,
            num_fibers, coat_speed, tower1, tower1_ent, tower2, tower2_ent, layer_type,
            op_init, amb_temp, amb_rh, notes
        ])
    ...
    try:
        ...
        st.markdown("### 📅 7-Day Review (Pilot Coating Process Tbl)")
        st.dataframe(recent if not recent.empty else pd.DataFrame(columns=pcoat_df.columns))
    except Exception as e:
        st.error(f"Could not load Pilot Coating review table: {e}")
...
with tab2:
    ...
    with st.form("tension_form", clear_on_submit=True):
        ...
        tension_val = st.number_input("Tension (g)", min_value=0.0, key="tens_val")
    ...
    try:
        ...
        # Note: No date column, using all entries (or modify if you add a date)
        st.markdown("### 📅 7-Day Review (Coater Tension Tbl)")
        st.dataframe(tens_df if not tens_df.empty else pd.DataFrame(columns=tens_df.columns))
    except Exception as e:
        st.error(f"Could not load Coater Tension review table: {e}")
...
with tab3:
    ...
    try:
        ...
        st.markdown("### 📅 7-Day Review (Coating Solution Mass Tbl)")
        st.dataframe(recent if not recent.empty else pd.DataFrame(columns=mass_df.columns))
    except Exception as e:
        st.error(f"Could not load Coating Solution Mass review table: {e}")
